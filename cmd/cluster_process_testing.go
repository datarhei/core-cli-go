...
var clusterProcessTestCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for i := 0; i < n; i++ {
			...
			if _, ok := processes[config.ID]; !ok {
				...
				if update {
					...
					if err := client.ClusterProcessUpdate(coreclient.NewProcessID(config.ID, config.Domain), config, false); err != nil {
						fmt.Printf("\nprocess %s (%4d / %4d) failed: %s\n", config.ID, i+1, n, err.Error())
					}
				}
		...
		for name := range processes {
			if err := client.ClusterProcessDelete(coreclient.NewProcessID(name, domain), true); err != nil {
...
func init() {
	...
	clusterProcessTestCmd.Flags().StringP("thumbs", "t", "", "template for thumbnail processes")
	clusterProcessTestCmd.Flags().BoolP("autostart", "a", true, "autostart processes")
	clusterProcessTestCmd.Flags().IntP("metadata", "m", 0, "metadata size")
}
