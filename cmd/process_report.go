...
var processReportCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if follow {
			...
			report, err := client.ProcessReport(id, createdAt, exitedAt)
			if err != nil {
				return err
			}
...
func init() {
	...
	processReportCmd.Flags().BoolP("follow", "f", false, "Follow the current report")
	processReportCmd.Flags().String("created", "", fmt.Sprintf("Show only reports that have been created at this time, format: %s", time.DateTime))
	processReportCmd.Flags().String("exited", "", fmt.Sprintf("Show only report that have exited at this time, format: %s", time.DateTime))
}
